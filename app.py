...
# ==================== HELPER FUNCTION ====================
def update_csv():
    """Auto update CSV file with latest database data (without adult_count, child_count)"""
    try:
...
@app.route('/reject-application/<int:app_id>', methods=['GET', 'POST'])
...
@app.route('/warden-check-in/<int:app_id>', methods=['GET', 'POST'])
...
@app.route('/warden-check-out/<int:app_id>', methods=['GET', 'POST'])
...
def inventory_page():
    ...
    # Calculate active guests (with allocated room and occupied)
    conn = get_db_connection()
    try:
        if USE_POSTGRES:
            import psycopg2.extras
            cursor = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
            cursor.execute('''
                SELECT COALESCE(SUM(guest_count) + COUNT(*), 0) as active 
                FROM applications 
                WHERE allocated_room IS NOT NULL AND room_status='Occupied'
            ''')
            active_guests = cursor.fetchone()['active']
        else:
            cursor = conn.cursor()
            cursor.execute('''
                SELECT COALESCE(SUM(guest_count) + COUNT(*), 0) as active 
